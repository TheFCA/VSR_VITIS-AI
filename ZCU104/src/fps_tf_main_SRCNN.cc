...
## Throughput test for the SRCNN network
...
#define KERNEL_CONV "SRCNN"
#define CONV_INPUT_NODE "srcnn_InputLayer_Conv2D"
#define CONV_OUTPUT_NODE "srcnn_OutputLayer_Conv2D"
...
inline void set_input_image(DPUTask *task, const string& input_node, const cv::Mat& image,int ind)
{
  ...
  normalize_image(image, data, scale); //image2
}
...
void run_CNN(DPUTask *taskConv, Mat img, int ind, std::vector<std::string> &images)
{
  assert(taskConv);
...
/**
 * @brief Run DPU CONV Task for SRCNN
 *
...
void superResolution(DPUKernel *kernelConv)
{
  ...
  workers[i] = thread([&,i]()
  {
	...
	run_CNN(taskConv, img, ind, images); //poner img2 
      }
...
/**
 * @brief Entry for running SRCNN network
 *
