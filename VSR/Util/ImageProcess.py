...
def array_to_img(x: np.ndarray, mode='RGB', min_val=0, max_val=255):
  ...
  if np.ndim(x) == 2:
    ...
    #return Image.fromarray(x.astype('float32'), mode='L').convert(mode) #fcarrio
  elif np.ndim(x) == 3:
    if DATA_FORMAT == 'channels_first':
      x = x.transpose([1, 2, 0])
    return Image.fromarray(x.astype('uint8'), mode=mode)
    #return Image.fromarray(x.astype('float32'), mode=mode)#fcarrio
  elif np.ndim(x) == 4:
    if DATA_FORMAT == 'channels_first':
      x = x.transpose([0, 2, 3, 1])
    ret = [Image.fromarray(np.round(i).astype('uint8'), mode=mode) for i in x]
    #ret = [Image.fromarray(np.round(i).astype('float32'), mode=mode) for i in x]#fcarrio
...
def img_to_array(img, data_format=None):
  ...
  #x = np.asarray(img, dtype=np.float32) # fcarrio added
  if len(x.shape) == 3:
    if data_format == 'channels_first':
      x = x.transpose([2, 0, 1])
  elif len(x.shape) == 2:
