...
def compat_param(par):
  ...
  if 'batch_shape' not in _par:
    ...
    # ------------- added multiscale fcarrio
#    if _par.multiscale==False:
#      patch_size = [x // _par.scale for x in to_list(patch_size, 2)]
#      batch_shape = [batch, *patch_size]
#    else:
    batch_shape = [batch,*to_list(patch_size,2)]
    # ------------------ fcarrio
    if DATA_FORMAT == 'channels_last': # that's for tensorflow
      batch_shape.append(channel)
    else: # that's for pytorch
