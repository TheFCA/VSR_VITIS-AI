...
#          - Adding Random Seed capability for reproducibility
import logging
...
from ..Backend.TF.Util import bicubic_rescale
...
def _augment(image, op):
  ...
  if op[1]: #flip / mirrow respect to y axis
    if DATA_FORMAT == 'channels_last':
      image = image[:, :, ::-1]
    else:
      image = image[..., ::-1]
  if op[2]:#flip / mirrow respect to x axis
    if DATA_FORMAT == 'channels_last':
      image = image[:, ::-1]
    else:
      image = image[:, :, ::-1]
...
class EpochIterator:
  ...
  def __init__(self, loader, shape, steps, shuffle=None):
    ...
    index_aux = self.index.copy() #fcarrio
    while (len(self.index) < self.steps * shape[0]) and self.index:
      self.index += self.index 
    if shuffle:
      self.loader.rs.shuffle(self.index)   
  ...
  def __next__(self):
    ...
    for i, d in self.index[slc]:
      ...
      if i >= len(self.loader.data['hr']):
        continue
      if self.loader.downscale:
        if self.loader.multiscale:
          self.loader.aux['scale'] = self.loader.rs.randint(2, 5) #fcarrio
        self.loader.add_data_transform('hr', Tidy(self.loader.aux['scale'], rs=self.loader.rs))
        if self.loader.upscale_model:
          self.loader.add_data_transform('lr', Tidy(self.loader.aux['scale'], rs=self.loader.rs))
        else:
          self.loader.add_data_transform('lr', Tidy(self.loader.aux['scale'], rs=self.loader.rs), Bicubic(1 / self.loader.aux['scale'], rs=self.loader.rs))
      ...
      for fn in cb_hr[0]:
        hr2 = [fn(img) for img in hr2] # PIL.Image.Image
      ...
      if not(self.loader.upscale_model):
        lr2 = [img.resize( ( int(img.size[0]*self.loader.aux['scale']) , int(img.size[1]*self.loader.aux['scale']) ),resample=Image.BICUBIC) for img in lr2] #fcarrio

      lr2 = [img.convert(self.loader.lr['color']) for img in lr2]

      #######################################################################################
      ########## CONVERTING TO NUMPY ARRAY AND CHECKING IF IT IS YCbCr
      #######################################################################################      
      if (self.loader.hr['color']=='YCbCr'):#fcarrio, adding Y 
        hr3 = np.stack([img_to_array(img, DATA_FORMAT)[:,:,0] for img in hr2])
        hr3 = np.expand_dims(hr3, axis=3)       
      else:
        hr3 = np.stack([img_to_array(img, DATA_FORMAT) for img in hr2])
      if (self.loader.lr['color']=='YCbCr'): #fcarrio, adding Y 
        lr3 = np.stack([img_to_array(img, DATA_FORMAT)[:,:,0] for img in lr2])
        lr3 = np.expand_dims(lr3, axis=3)       
      else:
      ...
      #######################################################################################
      ########## APPLYING TRANSFORMATION 2, if there are
      #######################################################################################            
      ...
      if self.loader.aux['augmentation']:
        ops = self.loader.rs.randint(0, 2, [3]) # to randomize the augmentation types that we will apply, fcarrio to rs
      else:
        ops = [0, 0, 0]

      _shape0 = hr4.shape # 1,patch_size, patch_size, channel
      _shape1 = lr4.shape # 1,patch_size//scale, patch_size//scale, channel
      hr5 = _augment(hr4.reshape([-1, *_shape0[-3:]]), ops) #,patch_size, patch_size, channel
      lr5 = _augment(lr4.reshape([-1, *_shape1[-3:]]), ops) #,patch_size//scale, patch_size//scale, channel
      del hr4, lr4
      ...
      if self.loader.upscale_model:
        ...
        im = Image.fromarray(np.uint8(lr5))
        im_resized = im.resize( ( int(im.size[0]/self.loader.aux['scale']) , int(im.size[1]/self.loader.aux['scale']) ),resample=Image.BICUBIC)
        lr5 = np.array(im_resized)
        lr5 = np.expand_dims(lr5, axis=0)       
        lr5 = np.expand_dims(lr5, axis=-1)       
        _shape1 = lr5.shape
...
class Loader(object):
  ...
  def __init__(self, hr_data, lr_data=None, scale=None, extra_data: dict = None, multiscale = False,
               upscale_model = False, threads=1,rs=None):
    # check type
    ...
    self.multiscale = multiscale or False #fcarrio
    self.upscale_model = upscale_model or False
    print ("upscale-model", self.upscale_model)
    self.downscale =  False #fcarrio
    ...
    self.fs = []
    self.loaded = 0
    if rs: #fcarrio, random seed generator
      self.rs = rs

    
    if self.hr['data'] is self.lr['data']:
      print ("self.hr['data'].capacity",self.hr['data'].capacity)
      cap = self.hr['data'].capacity
    else:
      print ("otroself.hr['data'].capacity",self.hr['data'].capacity)
    ...
    self.aux['cap'] = cap 
    if hr_data is lr_data and (scale > 1 or multiscale): 
      self.downscale = True
  ...
  def make_one_shot_iterator(self, batch_shape, steps, shuffle=None,
    ...
    if shape[-2] != -1 and self.crop is None:
      self.cropper(RandomCrop(self.aux['scale'],self.rs))
  ...
  def prefetch(self, shuffle=None, memory_usage=None):
    ...
    available_mem = max([np.uint64(memory_usage), np.uint64(FREE_MEMORY)]) #added by Fcarrio



    if not self.fs:
      if shuffle:
        self.aux['fetchList'] = list(self.rs.permutation(self.aux['fetchList'])) #fcarrio
      if available_mem > self.aux['cap']:
        ...
        prop = memory_usage / self.aux['cap'] / self.threads #free memory/requirement/threads, e.g. (1GB/2GB)/4 -> 0.125
        # How many frames can be read into memory each thread each epoch
        # Note: we assume each "frame" has a close size.
        # In case there is no enough memory, the dataset is reduced automatically ...
        n = max(1, int(np.round(len(self.hr['data']) * prop)))
        LOG.debug(f"Loading {prop * self.threads * 100:.4f}% data.")

        print ("reduced, I cannot with all the data, not enough memory")
