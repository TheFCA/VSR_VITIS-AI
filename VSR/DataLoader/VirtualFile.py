...
class RawFile(File): # probably not used, fcarrio
  ...
  def read_frame(self, frames=1, *args): # This is for video, fcarrio. Confirmed, not used
  ...
  def seek(self, offset, where=SEEK_SET):# This is for video, fcarrio. Confirmed, not used
    """Seek the position by `offset` relative to `where`.
    Args:
         offset: move the read pointer by `offset` bytes.
         where: same as io.SEEK_END, io.SEEK_CUR or io.SEEK_SET.
...
class ImageFile(File):
  ...
  def read_frame(self, frames=1, *args): # used!! fcarrio
    """read number `frames` of the file.
    Args:
        frames: number of frames to be loaded
    """
    image_bytes = [BytesIO(self.read()) for _ in range(frames)]
    container  = [Image.open(fp) for fp in image_bytes]
    return  container # here we open the files, fcarrio

  def read_frame2(self, frames=1, *args): # not used at this moment, fcarrio
  ...
  @property
  def shape(self): # this is only used to know the shape of the first pictures and then estimate the memory usage, fcarrio
    if self.file:
      file = self.file[0]
    else:
