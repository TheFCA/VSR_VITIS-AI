...
class Dataset(object):
  ...
  def compile(self):
    ...
    # image_nodes is a list of POSIX paths
    if not self.as_video:
      self.compiled = Container(sorted(image_nodes), self.as_video)
      return self.compiled # Este es nuestro caso, fcarrio
...
class Container(object):
  ...
  def __init__(self, urls, is_video: bool):
    ...
    pool = futures.ThreadPoolExecutor(4) #fcarrio before was 4
    fs = []
    self.nodes = []

    def _parse_image_node(url: Path):
      if url.is_dir(): # almacena en ImageFiles
  ...
  def capacity(self):
    ...
    for i, n in enumerate(self.nodes):
      ...
      if n.size() > max_sz:
        max_sz = n.size()
        pos = i
    shape = self.nodes[pos].shape # worst case
    max_bpp = 3
